...
class UnityIntegrationTest(
    ...
    @Test
    fun addArtistsToUnityAndGetThem() {
        ...
        val artistsOfUnityJson = makeGetRequest(mockMvc, "$unityEndpoint/$unitySavedId/artists", status().isOk)
        
        val artistsOfUnity = Json.decodeFromString<List<ArtistShortDto>>(artistsOfUnityJson)
        
        assertEquals(3, artistsOfUnity.size)
